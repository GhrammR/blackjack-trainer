...
10. **(Future capstone) Live Simulation mode.** Combines all three core skills into one continuous, unbroken task: keep the running count from a live seat-dealing table (step 3's mechanic), estimate decks remaining from the discard tray (step 4's mechanic), and compute true count — all at once, the way a real observer actually counts a table live. This is the mode that most closely mirrors the trainer's end goal. (This was "Option B" considered and deliberately deferred during step 4's design — promoted to a future capstone rather than discarded.) Build alongside step 8, after the core (steps 1-7) and the detection drill are solid.
