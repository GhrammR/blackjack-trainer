...
- **"Reset progress" resets v2 counting progress only, not the v1 strategy trainer's streak/stats.** `resetCountingProgress` in `persistence.ts` operates on a separate storage key (`double-down:counting:v1`) from the v1 `PersistedState` (`double-down:v1`), and the Settings panel that exposes the reset button lives inside the Card Counting Trainer specifically. A user resetting their counting personal bests would not expect that click to also wipe an unrelated 140-hand strategy streak. Settings (shoe size, seats, speed) are explicitly preserved across a reset — only personal bests and round/accuracy history clear — since those are stored preferences, not progress.
