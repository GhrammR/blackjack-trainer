...
- **Dealer's hole card is revealed at the same time as the upcard in the running-count drill.** `dealRound` in `countingDrill.ts` (v2 step 3) treats both dealer cards as visible together for counting purposes. In a real round, the hole card isn't exposed (and so isn't countable) until the dealer plays out the hand after all seats act — a true surveillance count would track that exact exposure timing. This drill simplifies to "deal phase only" rather than simulating full hand resolution (hits, dealer draws) across multiple seats, which would be considerably more machinery for a counting-mechanics drill. TODO: revisit at **step 8** (the counter-detection drill) — that's where full hand resolution and player count-dependent deviations actually matter, so hole-card exposure timing should be modeled faithfully there even if it stays simplified everywhere else.
...
- **Shoe countdown advances one card per press, ending on the press taken while viewing the last card.** `ShoeCountdownDrill.tsx` (v2 step 5): pressing Space/Enter or "Next card" while looking at card N reveals card N+1 (or ends the run if N is the last card) — so a full run takes exactly `shoe.length` presses, with no extra "reveal card 1" press needed (card 1 is shown immediately on Start) and no extra terminal press needed beyond the one taken on the last card. This was an inferred mechanic, not explicitly specified — chosen for the cleanest 1:1 mapping between "look at a card" and "press," which matches how a real speed-count drill feels. Personal bests only credit runs where the submitted count is correct (verified against `runningCount(shoe)`, not a hardcoded 0, so it stays robust under future non-whole-deck or non-Hi-Lo shoe configs); "Give up" abandons mid-run with no time recorded and no PB interaction.
